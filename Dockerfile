...
CMD ["sh", "-lc", "node openclaw.mjs config set gateway.controlUi.allowedOrigins '[\"https://openclaw-0322.onrender.com\"]' --strict-json; node openclaw.mjs config set agents.defaults.model.primary openai/gpt-4o-mini; node openclaw.mjs config set gateway.auth.token monsecret123; node openclaw.mjs config set gateway.auth.password monpassword123; exec node openclaw.mjs gateway --allow-unconfigured --bind=lan"]
